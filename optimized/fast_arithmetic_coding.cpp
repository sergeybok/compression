...
public:
    ...
    explicit InCacheString(const std::string& in, uint8_t cache, uint8_t cached_bits, size_t in_ptr) : in_(in), cache(cache), cached_bits(cached_bits), in_ptr(in_ptr) {};
...
py::bytes encode(
    ...
    // OutCacheString out_cache;
    // out_cache.out = state.out;  // initializing with the previously generated bytes
    // out_cache.cache = state.cache_encoder;
    // out_cache.count = state.count_encoder;
...
torch::Tensor decode(
    ...
    const std::string in = state.out_cache.out;
    const uint32_t c_count = 0x10000U;

#ifdef VERBOSE
    std::cout << "Decode value=" << value << " in_ptr=" << state.in_ptr <<std::endl;
#endif



    const int precision = 16;
    InCacheString in_cache(in, state.cache_decoder, state.cached_bits, state.in_ptr);   // Initialize with the stored buffer
    if (first_step) {
        in_cache.initialize(value);
    }

#ifdef VERBOSE
    std::cout << "Decode value=" << value << " c_count=" << c_count << " low=" << low <<" high=" << high << " cache=" << in_cache.cache << " cached_bits="<< in_cache.cached_bits << " in_ptr=" << in_cache.in_ptr <<std::endl;
#endif
...
#ifdef VERBOSE
    std::cout << "\tD value=" << value << " c_count=" << c_count << " low=" << low <<" high=" << high << " cache=" << in_cache.cache << " cached_bits="<< in_cache.cached_bits << " in_ptr=" << in_cache.in_ptr <<std::endl;
#endif
    state.low_decode = low;
    state.high_decode = high;
    state.cache_decoder = in_cache.cache;
    state.cached_bits = in_cache.cached_bits;
    state.in_ptr = in_cache.in_ptr;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    py::class_<CodecState>(m, "CodecState")
        .def(py::init<>())
        .def_readonly("out", &CodecState::out);
